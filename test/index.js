  ...
  tests: [
    {
      ...
      output: 'import _F from "folivora";\nconst x = _F.call([1, 2, 3]);'
    },
    {
    ...
    },
    {
      ...
      output: 'import { call } from "folivora";\nconst x = call([1, 2, 3], _x => filter(x => x % 2 == 0, _x), _x => map(x => x * 2, _x));'
    },
    {
      code:   'import { call } from "folivora";\ntCall([[1, 2, 3], [4, 5, 6]], map(xs => tCall(xs, filter(x => x % 2 == 0), map(x => x * 2))));',
      output: 'import { call } from "folivora";\ncall([[1, 2, 3], [4, 5, 6]], _x => map(xs => call(xs, _x2 => filter(x => x % 2 == 0, _x2), _x2 => map(x => x * 2, _x2)), _x));'
    },
    {
      code:   'import { call } from "folivora";\ntCall(1, multiple(2), x => dec(x, 3));',
      output: 'import { call } from "folivora";\ncall(1, _x => multiple(2, _x), _x => (x => dec(x, 3))(_x));'
    },
    {
      code:   'import { call } from "folivora";\ntCall(1, multiple(2), dec(_, 3));',
      output: 'import { call } from "folivora";\ncall(1, _x => multiple(2, _x), _x => dec(_x, 3));'
    },
    {
      code:   'import { call } from "folivora";\ntCall([1, 2, 3], filter(x => x % 2 == 0), first);',
      output: 'import { call } from "folivora";\ncall([1, 2, 3], _x => filter(x => x % 2 == 0, _x), _x => first(_x));'
    },
    {
      code:   'import { call } from "folivora";\ntCall([1, 2, 3], filter(x => x % 2 == 0), first());',
      output: 'import { call } from "folivora";\ncall([1, 2, 3], _x => filter(x => x % 2 == 0, _x), _x => first(_x));'
